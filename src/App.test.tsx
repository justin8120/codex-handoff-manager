...
describe("App", () => {
  ...
  test("switches inventory and task status filters", async () => {
    ...
    await user.click(within(inventorySection as HTMLElement).getByRole("button", { name: "待補齊" }));
  ...
  test("renders download actions", () => {
    ...
    expect(within(downloads as HTMLElement).getAllByRole("button", { name: "下載" }).length).toBeGreaterThan(0);
  });
});
